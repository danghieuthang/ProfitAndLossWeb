...
<template>
  <page-header-wrapper
  >
    <a-card :bordered="false" title="Transaction Info">
      <a-descriptions title="">
        ...
        <a-descriptions-item label="Transaction Type">{{ transaction['transaction-type'].code }} - {{ transaction['transaction-type'].name }}</a-descriptions-item>
        ...
        <a-descriptions-item label="Modifed Date">{{ transaction['modified-date'] }}</a-descriptions-item>
    ...
    <a-card
      ...
      <a-row :gutter="48">
        <a-col :md="8" :sm="24" v-for="item in receipt.evidences" :key="item.id">
  ...
  methods: {
    ...
    loadData () {
      ...
      TransactionRepository.searchById(id).then((res) => {
        ...
        this.transaction = rs
        ReceiptRepository.getEvidenceByTransactionId(id).then((res) => {
    ...
    close () {
      var path = this.$route.path.split('/transactions')[0]
      console.log(path)
